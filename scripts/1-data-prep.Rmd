...
```{r}
kc_zctas_2024 <- readRDS("../data/1-source/kc_zctas_2024.rds")
```
...
```{r}
mhsurv |>
  filter(!zip %in% kc_zctas_2024$GEOID20) |>
  nrow()
```
...
mhsurv <- mhsurv |>
  ...
  mutate(birth_sex = factor(birth_sex, levels = lvl))
```
...
```{r}
lvl <- c("Black", "White", "Other")

mhsurv <- mhsurv |>
  mutate(
    re_bwo = case_when(
      grepl("Asian|Hispanic|IndAm|Other|;", re_merge2) ~ lvl[3],
      grepl("AfAm|African", re_merge2) ~ lvl[1],
      grepl("MENA|White", re_merge2) ~ lvl[2]
    ),
    .after = re_merge2
  ) |>
...
```{r}
lvl <- c("Hispanic", "Not Hispanic")

mhsurv <- mhsurv |>
  mutate(
    hispanic_origin = if_else(re04 == 4, lvl[1], lvl[2], missing = lvl[2]),
    .after = re_bwo
  ) |>
  mutate(hispanic_origin = factor(hispanic_origin,levels = lvl))
```
...
```{r}
mhsurv <- mhsurv |>
  mutate(zip = factor(zip, levels = kc_zctas_2024$GEOID20))
```
...
Score Q13 according to the De Jong Gierveld Loneliness Scale.
...
```{r}
df <- mhsurv |>
  select(all_of(vars_emo), starts_with("lon_emo"))
```
...
```{r}
df <- mhsurv |>
  select(all_of(vars_soc), starts_with("lon_soc"))
```
...
```{r}
mhsurv$lon_total <- mhsurv$lon_emo + mhsurv$lon_soc
```
...
Factorize all numeric scores. Emotional loneliness has 6 questions, so scale is 0-6. Social loneliness has 5 questions, so scale is 0-5. Total loneliness has 11 questions, so scale is 0 to 11.

```{r}
mhsurv <- mhsurv |>
  mutate(
    lon_emo = if_else(lon_emo_missing == 0, lon_emo, NA),
    lon_soc = if_else(lon_soc_missing == 0, lon_soc, NA),
    lon_total = if_else(
      (lon_emo_missing + lon_soc_missing) %in% 0:1, lon_total, NA
    )
  ) |>
  mutate(across(
    c(lon_emo, lon_soc, lon_total),
    ~ factor(.x, levels = minmax_seq(.x)),
    .names = "{.col}_fac"
  ))
```
...
mhsurv <- mhsurv |>
  mutate(lon_total_bin = case_when(
    lon_total %in% 0:2 ~ lvl[1],
    lon_total %in% 3:8 ~ lvl[2],
    lon_total %in% 9:10 ~ lvl[3],
    lon_total == 11 ~ lvl[4],
    .default = NA
  )) |>
  mutate(lon_total_bin = factor(lon_total_bin, levels = lvl))
```

#### Binned scores (emotional & social)

```{r}
lvl <- c("Low", "Medium", "High")

mhsurv <- mhsurv |>
  mutate(lon_emo_bin = case_when(
    lon_emo %in% 0:1 ~ lvl[1],
    lon_emo %in% 2:3 ~ lvl[2],
    lon_emo %in% 4:6 ~ lvl[3],
    .default = NA
  )) |>
  mutate(lon_emo_bin = factor(lon_emo_bin, levels = lvl))
```
...
mhsurv <- mhsurv |>
  mutate(lon_soc_bin = case_when(
    lon_soc %in% 0:1 ~ lvl[1],
    lon_soc %in% 2:3 ~ lvl[2],
    lon_soc %in% 4:5 ~ lvl[3],
    .default = NA
  )) |>
  mutate(lon_soc_bin = factor(lon_soc_bin, levels = lvl))
```

```{r}
df <- mhsurv |>
  select(matches("^q13_|^lon_"))
```
