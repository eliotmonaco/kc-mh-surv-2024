...
Basic plots for each survey question.
...
plots <- lapply(names(mhsum), \(x) {
  ...
  if (plot == "bar") {
    ...
    fig <- mhsum[[x]] |>
      mutate(label = paste0(round_ties_away(pct), "%")) |>
      ggplot(aes(x = pct, y = prompt, label = label, fill = "#F04C43")) |>
      mh_barplot(legend = FALSE) +
...
<!-- # Q4 subquestions -->
<!-- lapply(names(plots_bar[grepl("^q4_", names(plots_bar))]), \(x) { -->
<!--   nm <- paste0("mh-report/plot-", x, ".svg") -->
<!--   ggsave(nm, plots_bar[[x]], width = 6.5, height = 3, bg = "white") -->
<!-- }) -->
...
<!-- # Q15 age differences -->
<!-- lapply(names(plots_age[grepl("^q15", names(plots_age))]), \(x) { -->
<!--   nm <- paste0("mh-report/plot-", x, "-age.svg") -->
<!--   ggsave(nm, plots_age[[x]], width = 4.5*1.5, height = 2*1.5, bg = "white") -->
<!-- }) -->
...
<!-- # Q11 -->
<!-- ggsave( -->
<!--   "mh-report/plot-q11.svg", -->
<!--   plot_q11, width = 7, height = 5, bg = "white" -->
<!-- ) -->
<!-- ``` -->
