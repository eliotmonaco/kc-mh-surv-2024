...
mhdemo$race_collapsed <- df |>
  select(race_collapsed, pct, pct_kc) |>
  pivot_longer(
    cols = !race_collapsed,
    names_to = "group",
    values_to = "pct"
  ...
  mutate(
    group = factor(group, levels = lvl),
    total = unique(df$total)
  )

mhdemo$race_collapsed
```
...
mhdemo$hispanic_origin <- df |>
  select(hispanic_origin, pct, pct_kc) |>
  pivot_longer(
...
```{r}
mhfigs <- imap(mhdemo, \(df, x) plot_sample(df, x))
names(mhfigs) <- names(mhdemo)

mhcaps <- imap(mhdemo, \(df, x) cap_text(df, x))
names(mhcaps) <- names(mhdemo)
```
...
```{r fig.width=5, fig.height=3}
mhfigs$race_ethnicity
```

```{r fig.width=5, fig.height=3.5}
mhfigs$race_collapsed
```

```{r fig.width=4, fig.height=3.5}
mhfigs$hispanic_origin
```
...
```{r fig.width=4, fig.height=6}
mhfigs$map_respondents <- ggplot(data = geo_kc) +
...
mhcaps$map_respondents <- paste0(
  "Approximate locations of Mental Health Survey respondents. N = 648."
)

mhfigs$map_respondents
```

## Tables

```{r}
tbl_dist <- mhgeo$district |>
  gt() |>
  cols_label(
...
mhcaps$tbl_dist <- "Survey respondents by council district"
...
```{r}
dir.create("mh-report/img", showWarnings = FALSE)
```

```{r}
ggsave(
  "mh-report/img/map-respondents.png", mhfigs$map_respondents,
  height = 6, dpi = 100
)

ggsave(
  "mh-report/img/demo-sex.png", mhfigs$birth_sex,
  width = 3, height = 3, dpi = 100
)

ggsave(
  "mh-report/img/demo-age.png", mhfigs$age,
  width = 5, height = 3, dpi = 100
)

ggsave(
  "mh-report/img/demo-age-str.png", mhfigs$age_str,
  width = 6, height = 3.5, dpi = 100
)

ggsave(
  "mh-report/img/demo-race-eth.png", mhfigs$race_ethnicity,
  width = 6, height = 4, dpi = 100
)

ggsave(
  "mh-report/img/demo-race-col.png", mhfigs$race_collapsed,
  width = 6, height = 4, dpi = 100
)

ggsave(
  "mh-report/img/demo-hisp.png", mhfigs$hispanic_origin,
  width = 4.5, height = 4, dpi = 100
)

gtsave(tbl_dist, "mh-report/img/tbl-dist.png")
```

```{r}
saveRDS(mhcaps, "../data/2-final/figure_captions_sample.rds")
```
