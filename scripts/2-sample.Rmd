...
acs1_city_2024 <- readRDS("../data/1-source/acs1_city_2024.rds")
acs5_zcta_2024 <- readRDS("../data/1-source/acs5_zcta_2024.rds")
```
...
```{r}
poptotal <- acs1_city_2024 |>
  filter(variable == "B01001_001") |>
  pull(estimate)

popsex <- acs1_city_2024 |>
...
```{r}
geo_zcta <- st_transform(kc_zctas_2024, 4326)

popzcta <- acs5_zcta_2024 |>
...
```{r}
geo_kc <- st_transform(kc_city_2024, crs = st_crs(3857))
ext <- st_bbox(geo_kc)
exp <- 1000
ext[1] <- ext[1] - exp
ext[2] <- ext[2] - exp
...
```{r fig.width=4, fig.height=6}
figs$map_resp <- ggplot(data = geo_kc) +
...
```{r}
ggsave(
  "report/img/map-resp.png", figs$map_resp,
  height = 6, dpi = 100
)

ggsave(
  "report/img/sex.png", figs$birth_sex,
  width = 3, height = 3, dpi = 100
)

ggsave( # pdf report
  "report/img/sex-pdf.png", figs$birth_sex,
  width = 3, height = 2.5, dpi = 100
)

ggsave(
  "report/img/age.png", figs$age,
  width = 5, height = 3, dpi = 100
)

ggsave(
  "report/img/age-str.png", figs$age_str,
  width = 6, height = 3, dpi = 100
)

ggsave(
  "report/img/race-eth.png", figs$race_ethnicity,
  width = 6, height = 4, dpi = 100
)

ggsave(
  "report/img/race-col.png", figs$race_collapsed,
  width = 6, height = 4, dpi = 100
)

ggsave(
  "report/img/hisp.png", figs$hispanic_origin,
